...
/**
 * from https://leetcode.com/problems/word-ladder-ii/
...
class Solution126 {
    fun findLadders(beginWord: String, endWord: String, wordList: List<String>): List<List<String>> {
        ...
        // check endWord is wordList
        if (!wordList.any { it == endWord }) {
            return result
        }

        val wordIndex = WordIndex.build(null, 0, wordList)

        // use Breadth First Search, use queue to store nodes of the same level
        // put beginWord as Root
        var queue = mutableListOf(WordNode(null, beginWord))

        while (queue.isNotEmpty() && result.isEmpty()) {
            val newQueue = mutableListOf<WordNode>()
            queue.forEach { node ->
                // remove the word already use, to speed up
                wordIndex.remove(node.word)

                for (index in 0 until wordLength) {
                    val foundWords = wordIndex.searchWords(node, index)

                    foundWords.forEach { word ->
                        val newNode = WordNode(node, word)
                        if (word == endWord) {
                            // if the word the same end Word, add to result
...
/**
 * The Word Index to increase the search speed
 * This class serves two types
 * Map<Char, WordIndex> if there are more then one words in the branch
 * Map<Char, Word> if there is only one word in the branch to save search
 * For example, ["hot", "dot", "dog", "lit", "log", "cog"] becomes similar like the below structure
 * {
 *   "h": "hot"
 *   "d": { "o" : { "t" : "dot" , "g" : "dog" }}
 *   "l": { "o" : "log", "i" : "lit"}
 ...
 */
class WordIndex(
    private val parent: WordIndex?,
    private val word: String?,
    private val index: MutableMap<Char, WordIndex>?,
    private val th: Int,
    ...
    companion object {
         ...
         * build Word Index
         */
        fun build(parent: WordIndex?, th: Int, wordList: List<String>): WordIndex {
            if (wordList.size == 1) {
                return WordIndex(parent, wordList.first(), null, th, true)
            } else {
                val words = mutableMapOf<Char, MutableList<String>>()
                val index = mutableMapOf<Char, WordIndex>()
                ...
                wordList.forEach { word ->
                    ...
                    if (words.contains(char)) {
                        ...
                        words[char] = mutableListOf(word)
                    }
                }

                return WordIndex(parent, null, index, th, false).apply {
                    words.forEach {
                        index[it.key] = build(this, th + 1, it.value)
                    }
                }
            }
        }
    ...
    /**
     * search words that match the condition, for example, the word is "hit" and the th is "1"
     * then find all the words match "h?t"
     */
    fun searchWords(node: WordNode, th: Int): List<String> {
        val word = node.word
        val queue = mutableListOf(this)
        val result = mutableListOf<String>()

        while (queue.isNotEmpty()) {
            val wordIndex = queue.removeAt(0)
            if (wordIndex.last) {
                if (node.checkWord(wordIndex.th, th, wordIndex.word!!)) {
                    result.add(wordIndex.word)
                }
            } else {
                if (wordIndex.th == th) {
                    // the replace th, add to
                    queue.addAll(wordIndex.index!!.values)
                } else {
                    wordIndex.index?.get(word[wordIndex.th])?.let {
                        queue.add(it)
                    }
                }
            }
    ...
    /**
     * remove the word from index and if it is the last one in tree, remove the parent from its parent as well
     */
    fun remove(word: String) {
        var node: WordIndex? = this
        var target: WordIndex? = null
        ...
        while (node != null) {
            if (node.last) {
                ...
                node = node.index?.get(word[node.th])
            }
        }
        ...
        if (target?.parent != null) {
            node = target.parent
            while (node != null) {
                val char = target.word!![node.th]
                node.index?.remove(char)

                if (node.index?.isEmpty() == true) {
                    // remove the parent from its parent, if there is no other nodes
                    node = node.parent
                } else {
                    node = null
                }
...
/**
 * The class to keep the track of breadth tree search result
 * */
class WordNode(private val parent: WordNode?, val word: String) {

    fun toList(): List<String> {
        val list = mutableListOf<String>()
        var node: WordNode? = this
    ...
    /**
     * check the word that match the condition, for example, the word is "hit" and the th is "1"
     * return true if the target is "h?t" like hot
     */
    fun checkWord(start: Int, th: Int, target: String): Boolean {
        for (i in start until target.length) {
            if (i == th) continue
...
// testing
fun test(beginWord: String, endWord: String, wordList: List<String>, expect: List<List<String>>) {
    val start = System.currentTimeMillis()
    val actual = Solution126().findLadders(beginWord, endWord, wordList)

    val spend = System.currentTimeMillis() - start

    println("spend: $spend ms, result:${actual.deepContentEquals(expect)}")
}
...
fun main() {
    ...
    test("hit", "cog", listOf("hot", "dot", "dog", "lot", "log"), listOf())
    ...
    test(
        ...
        listOf(
            listOf("sand","band","bans","bins","bids","aids","ands","ants","ante","anne","acne"),
            listOf("sand","band","bind","bins","bids","aids","ands","ants","ante","anne","acne"),
            listOf("sand","sane","sade","side","aide","aids","ands","ants","ante","anne","acne"),
            listOf("sand","sane","sine","side","aide","aids","ands","ants","ante","anne","acne"),
            listOf("sand","sans","bans","bins","bids","aids","ands","ants","ante","anne","acne"),
            listOf("sand","sans","kans","kins","kids","aids","ands","ants","ante","anne","acne"),
            listOf("sand","sans","sins","bins","bids","aids","ands","ants","ante","anne","acne"),
            listOf("sand","sans","sins","kins","kids","aids","ands","ants","ante","anne","acne"),
            listOf("sand","sans","sins","sims","aims","aids","ands","ants","ante","anne","acne"),
            listOf("sand","sans","sins","sims","aims","arms","arts","ants","ante","anne","acne"),
            listOf("sand","sans","sins","sirs","airs","aids","ands","ants","ante","anne","acne")
        )
    )
    // @formatter:on
}
